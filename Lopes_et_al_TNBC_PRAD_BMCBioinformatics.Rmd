---
title: "Extracting gene signatures from TNBC AR+ and prostate cancers"
...
tnbc_vars <- c('breast_carcinoma_estrogen_receptor_status',
               'breast_carcinoma_progesterone_receptor_status',
               'her2_immunohistochemistry_level_result',
               'lab_proc_her2_neu_immunohistochemistry_receptor_status',
               'lab_procedure_her2_neu_in_situ_hybrid_outcome_type'
)
clinical_tnbc <- clinical$primary.solid.tumor[, tnbc_vars]

clinical_tnbc_aux <- clinical_tnbc
names(clinical_tnbc_aux) <- c("ESR1","PGR","HER2_level","HER2_status","HER2_FISH")

summary(clinical_tnbc_aux)
clinical_tnbc_aux <- clinical_tnbc_aux[-(which(is.na(clinical_tnbc_aux[,1]))),]
```

Creating a vector of HER2 (IHC) status based on HER2 (ICH) level:
  
```{r}
clinical_tnbc_aux$my_HER2_level <- as.character(clinical_tnbc_aux$HER2_level)

clinical_tnbc_aux[which(clinical_tnbc_aux$HER2_level == '0'),6] <- 'Negative'
clinical_tnbc_aux[which(clinical_tnbc_aux$HER2_level == '1+'),6] <- 'Negative'
clinical_tnbc_aux[which(clinical_tnbc_aux$HER2_level == '2+'),6] <- 'Indeterminate'
clinical_tnbc_aux[which(clinical_tnbc_aux$HER2_level == '3+'),6] <- 'Positive'
```

Replacing HER2 (IHC) status by HER2 (FISH) (a more accurate test) whenever available:
  
```{r}
clinical_tnbc_aux$HER2_status_plus_FISH <- clinical_tnbc_aux[,4]
clinical_tnbc_aux[clinical_tnbc_aux[,5] !='',7] <- clinical_tnbc_aux[clinical_tnbc_aux[,5] !='',5]
```

Building the final clinical TNBC dataset, composed of all individuals and gene expression status of ESR1, PGR and HER2:
  
```{r}
clinical_tnbc <- clinical_tnbc_aux[,c(1,2,7)]

# replace all '' or NA with 'Indeterminate'
clinical_tnbc[is.na(clinical_tnbc) | clinical_tnbc == '' | clinical_tnbc == 'Equivocal'] <- 'Indeterminate'

# individuals with at least one 'Positive' and keep all are marked as not TNBC
tnbc_status_pos <- clinical_tnbc == 'Positive'
#tnbc.status.pos <- clinical.tnbc[,c(1,2,3)] == 'Positive'
not_tnbc_id <- rownames(tnbc_status_pos[which(sapply(seq(nrow(tnbc_status_pos)), function(ix) { any(tnbc_status_pos[ix,]) })),])

# individuals with at least one 'Indeterminate' and excluded from dataset
tnbc_status_ind <- clinical_tnbc[!(rownames(clinical_tnbc) %in% not_tnbc_id),] == 'Indeterminate'
not_tnbc_id     <- rownames(tnbc_status_ind[which(sapply(seq(nrow(tnbc_status_ind)), function(ix) { any(tnbc_status_ind[ix,]) })),])
clinical_tnbc <- clinical_tnbc[!(rownames(clinical_tnbc) %in% not_tnbc_id), ]

tnbc_status_neg <- clinical_tnbc == 'Negative'
tnbc_ix <- sapply(seq(nrow(clinical_tnbc)), function(ix) { all(tnbc_status_neg[ix,]) })

# set cases with TNBC
clinical_tnbc$tnbc <- 'NO_TNBC'
clinical_tnbc[tnbc_ix, 'tnbc'] <- 'TNBC'
```

Building the Y (binary data, i.e., TNBC vs. nonTNBC) response vector:
  
```{r}
ydata <- data.frame(tnbc = clinical_tnbc$tnbc, row.names = rownames(clinical_tnbc))
ydata$tnbc <- factor(ydata$tnbc)
```

Building the TNBC data:

```{r}
brca_tumor <- t(fpkm.per.tissue$primary.solid.tumor[,-which(duplicated(fpkm.per.tissue.barcode$primary.solid.tumor))])
common_rows <- which(getParticipantCode(rownames(brca_tumor)) %in% rownames(ydata))
brca_tumor <- brca_tumor[common_rows,]

ydata <- as.matrix(ydata)
ydata[ydata =="NO_TNBC"] <- 0
ydata[ydata =="TNBC"] <- 1
ydata <- as.numeric(ydata)

brca_tumor <- brca_tumor[which(ydata==1),]
ytnbc <- ydata[which(ydata==1)]

# considering only AR+ cases
brca_tumor_arplus_id <- which(brca_tumor[,"ENSG00000169083"] > median(brca_tumor[,"ENSG00000169083"]))

brca_tumor_arplus <- brca_tumor[brca_tumor_arplus_id,]
dim(brca_tumor_arplus)
# 80 TNBC tissue samples AR+
...
brca_tumor_arplus <- brca_tumor_arplus[which(getParticipantCode(rownames(brca_tumor_arplus)) %in% rownames(brca_survival_status)),]
brca_survival_status <- brca_survival_status[which(rownames(brca_survival_status) %in% getParticipantCode(rownames(brca_tumor_arplus))),]

brca_normal <- t(fpkm.per.tissue$solid.tissue.normal)
dim(brca_normal)
# 113 normal tissue samples

brca_data <- rbind(brca_tumor_arplus,brca_normal)
dim(brca_data)
# 193 is the total number of samples
```
...
ybrca <- c(rep(1,dim(brca_tumor_arplus)[1]),rep(0,dim(brca_normal)[1]))
...
dim(prad_data)
# 539 is the total number of samples
```
...
as.matrix(mcols(ensembl.protein.coding[colnames(xdata_less[,var_selected_netw_75_select[which(brcaprad_weights_netw[var_selected_netw_75_select] < 0.7)]])]))
...
# BRCA
xbrca_cox <- brca_tumor_arplus[,colnames(xdata_less)]
...
ggsurvplot(fit_KM,title="AR+ TNBC",font.main = c(16, "bold", "black"),legend.labs = c("low","high"), legend.title = "Risk group",pval=TRUE,palette = c("#009966","#FF3333"))

# PRAD
xprad_cox <- prad_tumor[,colnames(xdata_less)]
xprad_cox <- xprad_cox[,var_selected_netw_75_select]
...
prad_KM_mat <- as.data.frame(cbind(prad_survival_status[,1],prad_survival_status[,2],prad_fit_cox_risk_group))
...
ggsurvplot(fit_KM,title="PRAD",font.main = c(16, "bold", "black"),legend.labs = c("low","high"), legend.title = "Risk group",pval=TRUE,palette = c("#009966","#FF3333"))
```
